...
#if defined(__GLIBC__)
static void print_addr2line(const char *symbol, const void *address,
                            const int j, const void *offset) {
  // Only do this analysis for our own binary (skip trying to analyse libc.so,
  // etc.)
  ...
  snprintf(addr2line_cmd, sizeof(addr2line_cmd), "addr2line %p -e %.*s", addr,
           p, symbol);
  FILE *addr2line = NULL;
  char linebuffer[512];
  if (config.addr2line && (addr2line = popen(addr2line_cmd, "r")) != NULL &&
    ...
    snprintf(linebuffer, sizeof(linebuffer), "N/A (%p -> %s)", addr,
             addr2line_cmd);
  }
  // Log result
...
// Check GLIBC availability as MUSL does not support live backtrace generation
#if defined(__GLIBC__)
  // Try to obtain backtrace. This may not always be helpful, but it is better
  // than nothing
  void *buffer[255];
  ...
  for (int j = 0; j < calls; j++) {
    logg("B[%04i]: %s", j, bcktrace != NULL ? bcktrace[j] : "---");
  ...
  free(bcktrace);
#else
  logg("!!! INFO: pihole-FTL has not been compiled with glibc/backtrace "
       "support, not generating one !!!");
#endif
}

static void __attribute__((noreturn)) signal_handler(int sig, siginfo_t *si,
                                                     void *unused) {
  logg("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
       "!!!");
  logg("---------------------------->  FTL crashed!  "
  ...
  if (FTLstarttime != 0) {
    logg("FTL has been running for %lli seconds",
         (long long)time(NULL) - FTLstarttime);
  }
  ...
  if (sig == SIGSEGV) {
    switch (si->si_code) {
    case SEGV_MAPERR:
      logg("     with code:  SEGV_MAPERR (Address not mapped to object)");
      break;
    case SEGV_ACCERR:
      logg("     with code:  SEGV_ACCERR (Invalid permissions for mapped "
           "object)");
      break;
#ifdef SEGV_BNDERR
    case SEGV_BNDERR:
      logg("     with code:  SEGV_BNDERR (Failed address bound checks)");
      break;
#endif
#ifdef SEGV_PKUERR
    case SEGV_PKUERR:
      logg("     with code:  SEGV_PKUERR (Protection key checking failure)");
      break;
#endif
#ifdef SEGV_ACCADI
    case SEGV_ACCADI:
      logg("     with code:  SEGV_ACCADI (ADI not enabled for mapped object)");
      break;
#endif
#ifdef SEGV_ADIDERR
    case SEGV_ADIDERR:
      logg("     with code:  SEGV_ADIDERR (Disrupting MCD error)");
      break;
#endif
#ifdef SEGV_ADIPERR
    case SEGV_ADIPERR:
      logg("     with code:  SEGV_ADIPERR (Precise MCD exception)");
      break;
#endif
    default:
      logg("     with code:  Unknown (%i)", si->si_code);
      break;
    }
  }
  ...
  // Bus error - memory manager problem
    ...
    switch (si->si_code) {
    case BUS_ADRALN:
      logg("     with code:  BUS_ADRALN (Invalid address alignment)");
      break;
    case BUS_ADRERR:
      logg("     with code:  BUS_ADRERR (Non-existent physical address)");
      break;
    case BUS_OBJERR:
      logg("     with code:  BUS_OBJERR (Object specific hardware error)");
      break;
    case BUS_MCEERR_AR:
      logg("     with code:  BUS_MCEERR_AR (Hardware memory error: action "
           "required)");
      break;
    case BUS_MCEERR_AO:
      logg("     with code:  BUS_MCEERR_AO (Hardware memory error: action "
           "optional)");
      break;
    default:
      logg("     with code:  Unknown (%i)", si->si_code);
  ...
  // Illegal error - Illegal instruction detected
    ...
    switch (si->si_code) {
    case ILL_ILLOPC:
      logg("     with code:  ILL_ILLOPC (Illegal opcode)");
      break;
    case ILL_ILLOPN:
      logg("     with code:  ILL_ILLOPN (Illegal operand)");
      break;
    case ILL_ILLADR:
      logg("     with code:  ILL_ILLADR (Illegal addressing mode)");
      break;
    case ILL_ILLTRP:
      logg("     with code:  ILL_ILLTRP (Illegal trap)");
      break;
    case ILL_PRVOPC:
      logg("     with code:  ILL_PRVOPC (Privileged opcode)");
      break;
    case ILL_PRVREG:
      logg("     with code:  ILL_PRVREG (Privileged register)");
      break;
    case ILL_COPROC:
      logg("     with code:  ILL_COPROC (Coprocessor error)");
      break;
    case ILL_BADSTK:
      logg("     with code:  ILL_BADSTK (Internal stack error)");
      break;
#ifdef ILL_BADIADDR
    case ILL_BADIADDR:
      logg("     with code:  ILL_BADIADDR (Unimplemented instruction address)");
      break;
#endif
    default:
      logg("     with code:  Unknown (%i)", si->si_code);
  ...
  // Floating point exception error
    ...
    switch (si->si_code) {
    case FPE_INTDIV:
      logg("     with code:  FPE_INTDIV (Integer divide by zero)");
      break;
    case FPE_INTOVF:
      logg("     with code:  FPE_INTOVF (Integer overflow)");
      break;
    case FPE_FLTDIV:
      logg("     with code:  FPE_FLTDIV (Floating point divide by zero)");
      break;
    case FPE_FLTOVF:
      logg("     with code:  FPE_FLTOVF (Floating point overflow)");
      break;
    case FPE_FLTUND:
      logg("     with code:  FPE_FLTUND (Floating point underflow)");
      break;
    case FPE_FLTRES:
      logg("     with code:  FPE_FLTRES (Floating point inexact result)");
      break;
    case FPE_FLTINV:
      logg("     with code:  FPE_FLTINV (Floating point invalid operation)");
      break;
    case FPE_FLTSUB:
      logg("     with code:  FPE_FLTSUB (Subscript out of range)");
      break;
#ifdef FPE_FLTUNK
    case FPE_FLTUNK:
      logg(
          "     with code:  FPE_FLTUNK (Undiagnosed floating-point exception)");
      break;
#endif
#ifdef FPE_CONDTRAP
    case FPE_CONDTRAP:
      logg("     with code:  FPE_CONDTRAP (Trap on condition)");
      break;
#endif
    default:
      logg("     with code:  Unknown (%i)", si->si_code);
