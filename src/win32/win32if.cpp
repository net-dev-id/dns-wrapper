...
static IP_ADAPTER_ADDRESSES* getIfAddrs() {
    ...
    for (int i = 0; i < MAX_TRIES; i++) {
        ...
        res =
            GetAdaptersAddresses(AF_UNSPEC, flags, NULL, addrs, &outBufLen);
    ...
    if (res != NO_ERROR) {
        LERROR << "Call to GetAdaptersAddresses failed with error: " << res << std::endl;
...
template <> int Interface<IP_ADAPTER_ADDRESSES>::Index() const { return ptr->IfIndex; }

template <> std::string Interface<IP_ADAPTER_ADDRESSES>::Name() const {
    return ptr->AdapterName;
}

template <> Interface<IP_ADAPTER_ADDRESSES>& Interface<IP_ADAPTER_ADDRESSES>::operator++() {
    if (ptr) {
        ptr = ptr->Next;
    }
    return *this;
...
template <> Interface<IP_ADAPTER_ADDRESSES> Interface<IP_ADAPTER_ADDRESSES>::operator++(int) {
    if (ptr) {
        ptr = ptr->Next;
    }
    return *this;
...
template <>
NetInterface<IP_ADAPTER_ADDRESSES>::NetInterface() : start(getIfAddrs()), finish(nullptr) {}