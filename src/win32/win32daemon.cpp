...
Win32Daemon::Win32Daemon(HANDLE stopEvent) : stopEvent(stopEvent) {
  if (Args::Get()->configFile == CONFIG_FILE_PATH) {
    configReader = std::unique_ptr<Win32ConfigReader>(new Win32ConfigReader());
  } else {
    configReader = std::unique_ptr<IniConfigReader>(new IniConfigReader(Args::Get()->configFile));
  }
}
